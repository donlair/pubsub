...
Detailed type patterns and code examples for the Pub/Sub library.
...
|---------|----------|---------|
| `unknown` | Accepting arbitrary data (never `any`) | `function process(data: unknown): string` |
| Type guards | Runtime type checking | `value is PubSubMessage` |
| Branded types | Preventing ID confusion | `type MessageId = string & { __brand: 'MessageId' }` |
| `readonly` | Immutable properties | `readonly id: string` |
| `as const` | Literal object types | `const errors = { ... } as const` |
| Tuple returns | Admin operations `[result, metadata]` | `Promise<[Topic, any]>` |
| Method overloads | Type-safe event handlers | `on(event: 'message', ...)` |
